...
const CurrentlyReadingScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchCurrentlyReading = async () => {
      try {
        ...
        if (currentlyReading) {
          setCurrentlyReading(JSON.parse(currentlyReading));
        }
      } catch (error) {
        console.error(error);
      }
  ...
  const moveToCategory = async (category) => {
    try {
      let updatedCurrentlyReading = currentlyReading.filter(item => item.id !== selectedBook.id);
      await AsyncStorage.setItem('currentlyReading', JSON.stringify(updatedCurrentlyReading));
